...
// Centralized borrowing rule — 7 days
  ...
  if (success) return (
    <div className="ib">
      <div className="ib-success">
        ...
        <p className="ib-success-dates">Issue Date: {formatDate(issueDate)} &nbsp;|&nbsp; Due Date: {formatDate(dueDate)}</p>
        <button className="ib-new-btn" onClick={() => { setStep(1); setPhone(''); setMember(null); setSelectedBook(null); setSuccess(false) }}>
          Issue Another Book
        </button>
      </div>
    </div>
          ...
          <div className="ib-card">
            <div className="ib-found-member">
              ...
              <div>
                ...
                <div className="ib-member-meta">{member.id} · {member.phone}</div>
              </div>
            ...
            <div className="ib-book-list">
              {mockBooks.filter(b => b.title.toLowerCase().includes(bookSearch.toLowerCase()) || !bookSearch).map(b => (
                <div key={b.id}
            ...
            {selectedBook && (
              <button className="ib-btn ib-btn--primary" style={{ marginTop: 14 }} onClick={() => setStep(3)}>
                Continue →
              </button>
            )}
          </div>
